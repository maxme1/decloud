<script lang="ts">
    ...
    function count(msgs: Message[]) {
        ...
        const colors = allUsers.map((_) => getRandomColor());
        const series = allUsers.map((user, index) => {
            const dayCount = realMessages
                ...
                .reduce(
                    (acc, message) => {
                        ...
                        const index = acc.findIndex((item) => item.x === day);
                        if (index !== -1) {
                            acc[index].y += 1;
                        }
                        return acc;
